# ================================================================================
# Aspose.PSD 일반 레이어 콘텐츠 교체 및 리사이즈 테스트
# (test-replace-regular-layer-with-half-size-ok.ps1)
# ================================================================================
...
try {
    ...
    Write-Host "   - 원본 PNG 로드 완료: $($pngImage.Width)x$($pngImage.Height)"

    # --- [핵심] PNG 이미지 리사이즈 ---
    Write-Host "2. PNG 이미지 크기를 50%로 줄이는 중..." -ForegroundColor Cyan
    $newWidth = [int]($pngImage.Width / 2)
    $newHeight = [int]($pngImage.Height / 2)
    
    # Resize 메서드를 사용하여 이미지 크기 변경
    $pngImage.Resize($newWidth, $newHeight)
    Write-Host "   - PNG 리사이즈 완료: $($pngImage.Width)x$($pngImage.Height)"

    # --- 대상 레이어 찾기 ---
    Write-Host "3. 대상 레이어 '$TargetLayerName' 찾는 중..." -ForegroundColor Cyan
    ...
    # --- 픽셀 데이터 교체 ---
    Write-Host "4. 픽셀 데이터 교체 시작..." -ForegroundColor Cyan
    
    # 1. 크기가 조절된 PNG 이미지에서 픽셀 데이터를 가져옵니다.
    $pngRect = New-Object Aspose.PSD.Rectangle(0, 0, $pngImage.Width, $pngImage.Height)
    $pngPixels = $pngImage.LoadArgb32Pixels($pngRect)
    
    # 2. 대상 레이어의 크기를 리사이즈된 PNG 이미지 크기와 동일하게 조정합니다.
    ...
    # --- PSD 파일 저장 ---
    Write-Host "5. 변경된 PSD 파일 저장 중..." -ForegroundColor Cyan
    ...
    Write-Host "6. 모든 리소스를 정리했습니다." -ForegroundColor Gray
}