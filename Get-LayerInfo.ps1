...
function Load-AsposePSD {
    ...
    if (-not (Test-Path $asposePsdDll)) {
        ...
        try {
            ...
            dotnet new console -n TempProject -o $tempDir --force
            Set-Location $tempDir
            
            dotnet add package Aspose.PSD --version 24.12.0
            dotnet add package Aspose.Drawing --version 24.12.0
            ...
            if ($asposePsdPath) {
                Copy-Item $asposePsdPath.FullName "$packageDir/Aspose.PSD.dll" -Force
                Write-Host "✓ Aspose.PSD.dll copied"
            }
            ...
            if ($asposeDrawingPath) {
                Copy-Item $asposeDrawingPath.FullName "$packageDir/Aspose.Drawing.dll" -Force
                Write-Host "✓ Aspose.Drawing.dll copied"
            }
            ...
            foreach ($dep in $dependencies) {
                ...
                if ($depPath) {
                    Copy-Item $depPath.FullName $packageDir -Force
                    Write-Host "✓ $($depPath.Name) copied"
                }
            }
...
function Main {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "Layer Statistics:" -ForegroundColor Yellow
        Write-Host "  Total: $($stats.Total)"
        Write-Host "  Ignore: $($stats.Ignore)"
        Write-Host "  Groups: $($stats.Groups)"
        Write-Host "  Layers: $($stats.Layers)"
