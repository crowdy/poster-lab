    ...
    try {
        ...
        try {
            ...
            if ($tempPsd.Layers.Length -gt 0) {
                $layer = $tempPsd.Layers[0]
                
                # 새로운 레이어 생성 (복사하지 않고 새로 생성)
                $newLayer = [Aspose.PSD.FileFormats.Psd.Layers.Layer]::new($layer.Width, $layer.Height)
                $newLayer.Name = $LayerName
                $newLayer.DisplayName = $LayerName
                
                # 픽셀 데이터 복사
                $pixels = $layer.LoadArgb32Pixels($layer.Bounds)
                $newLayer.SaveArgb32Pixels($layer.Bounds, $pixels)
...
function Create-LayerFromPNG-Method3 {
    ...
    try {
        ...
        if ($pngImage -is [Aspose.PSD.RasterImage]) {
            ...
            # 빈 레이어 생성
            $layer = [Aspose.PSD.FileFormats.Psd.Layers.Layer]::new($rasterImage.Width, $rasterImage.Height)
            $layer.Name = $LayerName
            $layer.DisplayName = $LayerName
            
            # 픽셀 데이터 복사
            $pixels = $rasterImage.LoadArgb32Pixels($rasterImage.Bounds)
            $layer.SaveArgb32Pixels($rasterImage.Bounds, $pixels)
