...
function Bake-PSD {
    ...
    try {
        ...
        if ($psdImage -is [Aspose.PSD.FileFormats.Psd.PsdImage]) {
            ...
            # ===== Smart Object 처리 로직 구현 =====
            if ($Config_EmbedLinkedSmartObjects) {
                Write-PSDLog "DEBUG" "Processing smart objects (convert linked PNGs -> embedded)"
                $embeddedCount = 0
                $relinkedAndEmbedded = 0
                $skipped = 0

                for ($li = 0; $li -lt $psdImage.Layers.Count; $li++) {
                    $layer = $psdImage.Layers[$li]
                    if ($layer -is [Aspose.PSD.FileFormats.Psd.Layers.SmartObjects.SmartObjectLayer]) {
                        $so = [Aspose.PSD.FileFormats.Psd.Layers.SmartObjects.SmartObjectLayer]$layer
                        $ctype = $so.ContentType
                        $lname = if ($so.DisplayName) { $so.DisplayName } else { "SmartObject#$li" }
                        Write-PSDLog "DEBUG" ("SmartObject[{0}] '{1}' ContentType={2}" -f $li, $lname, $ctype)

                        try {
                            switch ($ctype) {
                                ([Aspose.PSD.FileFormats.Psd.Layers.SmartObjects.SmartObjectType]::Embedded) {
                                    # 이미 임베드된 상태
                                    $skipped++
                                    Write-PSDLog "DEBUG" ("'{0}' is already embedded; skip" -f $lname)
                                }

                                ([Aspose.PSD.FileFormats.Psd.Layers.SmartObjects.SmartObjectType]::AvailableLinked) {
                                    # 링크가 살아있으니 즉시 임베드
                                    $so.EmbedLinked()               # 링크 -> 임베드
                                    $so.UpdateModifiedContent()     # 캐시 반영
                                    $embeddedCount++
                                    Write-PSDLog "INFO" ("Embedded linked smart object: '{0}'" -f $lname)
                                }

                                ([Aspose.PSD.FileFormats.Psd.Layers.SmartObjects.SmartObjectType]::UnavailableLinked) {
                                    # 링크가 깨졌다면 작업 폴더의 PNG로 재연결 후 임베드
                                    $candidate = $null

                                    # 1) LinkDataSource에서 기대 파일명을 추출해 우선 매칭 (가능한 경우)
                                    try {
                                        $lds = $so.ContentsSource  # LinkDataSource
                                        if ($lds -and $lds.FileName) {
                                            $expected = $lds.FileName
                                            $candidate = Get-ChildItem -LiteralPath $workingPath -Filter $expected -File -ErrorAction SilentlyContinue | Select-Object -First 1
                                            if (-not $candidate) {
                                                $base = [System.IO.Path]::GetFileNameWithoutExtension($expected)
                                                $candidate = Get-ChildItem -LiteralPath $workingPath -Filter ($base + ".png") -File -ErrorAction SilentlyContinue | Select-Object -First 1
                                            }
                                        }
                                    } catch { }

                                    # 2) 후보가 없으면 폴더 내 PNG 1개뿐이면 그걸 사용
                                    if (-not $candidate) {
                                        $pngs = Get-ChildItem -LiteralPath $workingPath -Filter "*.png" -File -ErrorAction SilentlyContinue
                                        if ($pngs.Count -eq 1) { $candidate = $pngs[0] }
                                        elseif ($so.DisplayName) {
                                            # 3) 레이어 표시명과 파일명 유사 매칭
                                            $candidate = $pngs | Where-Object {
                                                $_.BaseName -ieq $so.DisplayName -or
                                                $_.Name -ieq $so.DisplayName -or
                                                $_.BaseName -ilike ($so.DisplayName + "*")
                                            } | Select-Object -First 1
                                        }
                                    }

                                    if ($candidate) {
                                        Write-PSDLog "DEBUG" ("Relinking '{0}' -> {1}" -f $lname, $candidate.Name)
                                        $so.RelinkToFile($candidate.FullName)  # 링크 갱신
                                        $so.EmbedLinked()                       # 임베드로 변환
                                        $so.UpdateModifiedContent()
                                        $relinkedAndEmbedded++
                                        Write-PSDLog "INFO" ("Relinked and embedded: '{0}'" -f $lname)
                                    } else {
                                        $skipped++
                                        Write-PSDLog "WARN" ("No matching PNG found in working folder for smart object '{0}'. Leaving as 'UnavailableLinked'." -f $lname)
                                    }
                                }

                                default {
                                    $skipped++
                                    Write-PSDLog "WARN" ("Unknown SmartObjectType {0} on layer '{1}'" -f $ctype, $lname)
                                }
                            }
                        } catch {
                            Write-PSDLog "ERROR" ("Smart object processing failed for '{0}': {1}" -f $lname, $_.Exception.Message)
                            throw
                        }
                    }
                }

                Write-PSDLog "INFO" ("Smart objects processed - Embedded: {0}, Relinked+Embedded: {1}, Skipped: {2}" -f $embeddedCount, $relinkedAndEmbedded, $skipped)
            }
            # ===== Smart Object 처리 로직 끝 =====
