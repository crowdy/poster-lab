            ...
            if ($convertedCount -gt 0) {
                ...
                try {
                    ...
                    if (Test-Path $outputPsdPath) {
                        ...
                        if ($DebugMode) {
                            Write-Host "    ✓ Saved PSD ($([math]::Round($fileSize/1MB, 2)) MB) with $convertedCount transparent smart objects"
                        } else {
...
# ===== 메인 실행 =====
$startTime = Get-Date
Write-Host "=== PSD Templatization v2 (with Transparent Replacement) Started at $($startTime.ToString('yyyy-MM-dd HH:mm:ss')) ==="
...
Write-Host ""
Write-Host "=== PSD Templatization v2 Completed ==="
...
Write-Host ""
Write-Host "Key changes in v2:"
Write-Host "- Original PSD copied to working directory before processing"
Write-Host "- Target layers replaced with transparent content before Smart Object conversion"
Write-Host "- Extended layer patterns: machine-frame, machine-icon #2, smart-icon, machine_main"
