...
function Get-MachineCountFromCSV {
    ...
    try {
        ...
        foreach ($row in $csv) {
            if ($DebugMode) { 
                Write-Host "DEBUG: Processing row - UUID: '$($row.FileNameWithoutExtension)', MachineCount: '$($row.MachineCount)'"
            }
            
            if ($row.FileNameWithoutExtension -and $row.MachineCount) {
                $machineCount[$row.FileNameWithoutExtension] = [int]$row.MachineCount
                if ($DebugMode) { 
                    Write-Host "DEBUG: Added $($row.FileNameWithoutExtension) -> $($row.MachineCount) machines"
                }
            }
        }
        
        Write-Host "Loaded machine counts for $($machineCount.Count) UUIDs"
        
        if ($DebugMode) {
            Write-Host "DEBUG: Machine count summary:"
            $machineCount.GetEnumerator() | Sort-Object Key | ForEach-Object {
                Write-Host "  $($_.Key) = $($_.Value) machines"
            }
        }
...
function Process-PsdForMachines {
    ...
    if ($DebugMode) { 
        Write-Host "Processing UUID: $UUID with $MachineCount machines"
    ...
    for ($machineNum = 1; $machineNum -le $MachineCount; $machineNum++) {
        $machineDir = Join-Path $TemplateRoot "machine_$machineNum"
...
# 머신 개수 정보 로드
$machineCount = Get-MachineCountFromCSV -CsvPath $csvPath -DebugMode:$Debug
if ($machineCount.Count -eq 0) {
    Write-Error "No machine count data loaded"
    exit 1
}
...
foreach ($psdFile in $psdFiles) {
    ...
    if (-not $machineCount.ContainsKey($uuid)) {
        Write-Warning "No machine count data for UUID: $uuid (skipping)"
        $errorCount++
        continue
    }
    
    $machines = $machineCount[$uuid]
    Write-Host "[$processedCount/$($psdFiles.Count)] Processing $uuid ($machines machines)..."
    
    try {
        if (Process-PsdForMachines -SourcePsdPath $psdFile.FullName -UUID $uuid -MachineCount $machines -TemplateRoot $templateRoot -WhatIfMode:$WhatIf -DebugMode:($Verbose -or $Debug)) {
            $successCount++
